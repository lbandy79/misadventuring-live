...
export default function DecoderRingDisplay(_props: Props) {
  ...
  const handleStatusTransition = (_from: string, to: string, data: DecoderRingState) => {
    ...
    // Character vote opens → keep wheel spinning
    if (to === 'voting-character') {
      audioMixer.play('whoosh');
      startWheelSpin();
    }
  ...
  const settleWheel = (_year: number) => {
    if (!wheelRef.current) return;
  ...
  const revealCharacterCard = (_charId: string) => {
  ...
  return (
    <div className={`decoder-display decoder-display--${theme.id}`} ref={containerRef}>
      ...
      {/* ═══ IDLE STATE (waiting between spins) ═══ */}
      <AnimatePresence>
        {state.status === 'idle' && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <div className="idle-ring-glow" />
            <h2 className="idle-title">🔮 The Well of Lines</h2>
            <p className="idle-subtitle">
              {state.crew.length === 0
                ? 'The Well awakens\u2026'
                : `Spin ${state.spinNumber} \u2014 ${state.crew.length} crew recruited`}
            </p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {(state.status === 'voting-year' || state.status === 'voting-character' || state.status === 'revealing-character') && (
          <motion.div
      ...
      {/* ═══ CHARACTER VOTE DISPLAY (when multiple chars at winning year) ═══ */}
      <AnimatePresence>
        {state.status === 'voting-character' && state.winningYear && (
          <motion.div
            className="char-vote-display"
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0 }}
          >
            <h3 className="role-vote-title">Year {state.winningYear} \u2014 Who Emerges?</h3>
            <div className="role-vote-bars">
              {getCharacterByYear(state.winningYear).filter(c => !c.isNPC).map(c => {
                const count = state.characterVotes[c.id] || 0;
                const total = Object.values(state.characterVotes).reduce((a, b) => a + b, 0);
                const pct = total > 0 ? (count / total) * 100 : 0;
                return (
                  <div key={c.id} className="role-bar-row">
                    <span className="role-bar-label">
                      <img
                        src={`/images/ip-isle/${c.image}`}
                        alt={c.name}
                        className="char-vote-thumb"
                        onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }}
                      />
                      {c.name}
                    </span>
      ...
      {/* ═══ COMPLETE BANNER ═══ */}
      <AnimatePresence>
        {state.status === 'complete' && revealedChar && state.winningRole && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
          >
            <h2 className="complete-title">✅ Crew Updated!</h2>
            <p className="complete-assignment">
              {revealedChar.name} → {SHIP_ROLES.find(r => r.id === state.winningRole)?.emoji}{' '}
              {SHIP_ROLES.find(r => r.id === state.winningRole)?.label}
            </p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {(state.status === 'voting-year' || state.status === 'voting-character' || state.status === 'voting-role') && (
